...
import CustomerMonthlySalesHeader from "./CustomerMonthlySalesHeader";
...
const CustomerMonthlySalesComponent = () => {
  ...
  const [getMonthlyCustomerSalesPivot] = useLazyQuery(
    GET_CUSTOMER_MONTHLY_SALES_PIVOT_QUERY
  );
  ...
  const handleOnGridReady = (
    params: GridReadyEvent<CustomerSalesSummary>
  ) => {
    setGridReady(true);
  ...
  const datasource = useMemo(
        ...
        const result = await handleTryCatch(async () => {
          ...
          if (data.getMonthlyCustomerSalesPivot) {
            const {
              ...
              total,
              totalsRow,
            } = data.getMonthlyCustomerSalesPivot;
            params.success({
              rowData: rows,
              rowCount: total,
            });
    ...
    [parsedStoreId, selectedOutlet, dispatch, getMonthlyCustomerSalesPivot, debouncedSearch]
  );
