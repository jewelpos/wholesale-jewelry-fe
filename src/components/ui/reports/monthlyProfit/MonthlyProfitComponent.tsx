...
import { monthlyProfitColumnDefs } from "./ColumnDef";
import { GET_MONTHLY_SALES_PROFIT_PIVOT_QUERY } from "@/lib/graphql/query/reports";
...
const MonthlyProfitComponent = () => {
  const { storeId: storeIdParam } = useParams();
  const parsedStoreId = parseInt(storeIdParam as string, 10);
  const [getMonthlySalesProfitPivot] = useLazyQuery(GET_MONTHLY_SALES_PROFIT_PIVOT_QUERY);
  ...
  const [totals, setTotals] = useState<Partial<WarehouseSalesSummary> | null>(null);
  const [yearRange, setYearRange] = useState<[number, number]>([YEAR_MAX, YEAR_MAX]);

  const handleOnGridReady = (params: GridReadyEvent<WarehouseSalesSummary>) => {
    setGridReady(true);
  ...
  const datasource = useMemo(
    () => ({
      getRows: async (params: IServerSideGetRowsParams) => {
        const filters = filterVariables(params, debouncedSearch, "warehousename");
        ...
        const result = await handleTryCatch(async () => {
          const { data } = await getMonthlySalesProfitPivot({
            variables: { storeid: parsedStoreId, outletid: selectedOutlet, ...filters },
          });
          if (data.getMonthlySalesProfitPivot) {
            const { data: rows, total, totalsRow } = data.getMonthlySalesProfitPivot;
            ...
            if (!rows.length) {
              ...
              if (totalsRow) {
                setTotals(totalsRow);
                const pinnedRow: Partial<WarehouseSalesSummary> = {
    ...
    [parsedStoreId, selectedOutlet, dispatch, getMonthlySalesProfitPivot, debouncedSearch, yearRange]
  );
  ...
  const { isAdmin, isCollapsed, toggle } = useSummaryPanel("monthly-profit");

  const summaryCards: SummaryCardDef[] = [
    { label: "Total Revenue", value: totals?.total_sales, format: "currency", accent: "#6366f1" },
    ...
    <ReportLayout>
      ...
      {totals && (
        <ReportMiniChart
          ...
          subtitle="Total sales by month - profit analysis"
          color="#6366f1"
          type="area"
        />
      )}
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minHeight: 0 }}>
            <POSGrid gridKey="report-monthly-profit"
              ref={gridRef}
              columnDefs={monthlyProfitColumnDefs}
              fillHeight
...
export default MonthlyProfitComponent;
