...
const MonthlyProfitComponent = () => {
  ...
  const [getMonthlySalesProfitPivot] = useLazyQuery(
    GET_MONTHLY_SALES_PROFIT_PIVOT_QUERY
  );
  ...
  const handleOnGridReady = (
    params: GridReadyEvent<WarehouseSalesSummary>
  ) => {
    setGridReady(true);
  ...
  const datasource = useMemo(
        ...
        const result = await handleTryCatch(async () => {
          ...
          if (data.getMonthlySalesProfitPivot) {
            const {
              ...
              total,
              totalsRow,
            } = data.getMonthlySalesProfitPivot;
            params.success({
              rowData: rows,
              rowCount: total,
            });
    ...
    [parsedStoreId, selectedOutlet, dispatch, getMonthlySalesProfitPivot, debouncedSearch]
  );
