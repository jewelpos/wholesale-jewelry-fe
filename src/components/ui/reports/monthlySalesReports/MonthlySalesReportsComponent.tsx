...
import { WarehouseYearlySummary } from "@/types/reports";
import { monthlySalesReportsColumnDefs } from "./ColumnDef";
...
const MonthlySalesReportsComponent = () => {
  const { storeId: storeIdParam } = useParams();
  const parsedStoreId = parseInt(storeIdParam as string, 10);
  const [getMonthlySalesPivot] = useLazyQuery(GET_MONTHLY_SALES_PIVOT_QUERY);
  ...
  const [totals, setTotals] = useState<Partial<WarehouseYearlySummary> | null>(null);
  const [yearRange, setYearRange] = useState<[number, number]>([YEAR_MAX, YEAR_MAX]);

  const handleOnGridReady = (params: GridReadyEvent<WarehouseYearlySummary>) => {
    setGridReady(true);
  ...
  const datasource = useMemo(
    () => ({
      getRows: async (params: IServerSideGetRowsParams) => {
        ...
        const result = await handleTryCatch(async () => {
          const { data } = await getMonthlySalesPivot({
            variables: { storeid: parsedStoreId, outletid: selectedOutlet, ...filters },
          });
          if (data.getMonthlySalesPivot) {
            const { data: rows, total, totalsRow } = data.getMonthlySalesPivot;
    ...
    [parsedStoreId, selectedOutlet, dispatch, getMonthlySalesPivot, debouncedSearch, yearRange]
  );
  ...
  const { isAdmin, isCollapsed, toggle } = useSummaryPanel("monthly-sales");

  const summaryCards: SummaryCardDef[] = [
    { label: "Total Sales", value: totals?.total_sales, format: "currency", accent: "#6366f1" },
    { label: "Total Cost", value: totals?.total_cost, format: "currency", accent: "#0ea5e9" },
    { label: "Gross Profit", value: totals?.total_profit, format: "currency", accent: "#10b981" },
    { label: "Margin", value: totals?.profit_margin_percent, format: "percent", accent: "#8b5cf6" },
  ];
    ...
    <ReportLayout>
      ...
      {totals && (
        <ReportMiniChart
          ...
          subtitle="Total sales across all warehouses by month"
          color="#6366f1"
          type="area"
        />
      )}
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minHeight: 0 }}>
            <POSGrid gridKey="report-monthly-sales"
              ref={gridRef}
              columnDefs={monthlySalesReportsColumnDefs}
              fillHeight
...
export default MonthlySalesReportsComponent;
