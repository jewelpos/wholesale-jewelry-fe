...
const SalesOrderEmailModal = ({
  ...
  const handleSend = async () => {
    if (inputValue.trim()) {
      const ok = addEmail(inputValue);
      if (!ok) return;
    }
    const finalEmails = inputValue.trim()
      ? [...new Set([...emails, ...inputValue.split(/[,;\s]+/).map(e => e.trim()).filter(Boolean)])]
      : emails;
      ...
      <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: 480 }}>
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Email Sales Order{salesOrderNumbers.length > 1 ? "s" : ""}</h5>
            <button type="button" className="btn-close" onClick={onClose} />
          </div>
          <div className="modal-body">
            <p className="text-muted small mb-2">
              SO #{salesOrderNumbers.join(", ")} — PDF will be sent as attachment
            </p>
...
export default SalesOrderEmailModal;
